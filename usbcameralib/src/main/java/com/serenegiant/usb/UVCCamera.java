...
public class UVCCamera {
    ...
    // until here
    /**
     * the sonctructor of this class should be ic_call_logo within the thread that has a looper
...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
    ...
    public synchronized void destroy() {
    	Log.d(TAG,"destroy camera begin");
    	close();
		Log.d(TAG,"destroy camera close-1");
    	if (mNativePtr != 0) {
    		nativeDestroy(mNativePtr);
    		mNativePtr = 0;
    	}
		Log.d(TAG,"destroy camera nativeDestroy-1");
    }

    // wrong result may return when you ic_call_logo this just after camera open.
...
//**********************************************************************
    /**
     * start movie capturing(this should ic_call_logo while previewing)
     * @param surface
     */
