...
import com.hv.calllib.util.SysAdaptation;
import com.hv.calllib.util.Utilitys;

/**
 * AppRTCAudioManager manages all audio related parts of the AppRTC demo.
 */
public class PeerAudioManager {
  private static final String TAG = "PeerAudioManager";
  ...
  private void onProximitySensorChangedState() {
    ...
    if (audioDevices.size() == 2 && audioDevices.contains(PeerAudioManager.AudioDevice.EARPIECE)
        && audioDevices.contains(PeerAudioManager.AudioDevice.SPEAKER_PHONE)) {
      if (proximitySensor.sensorReportsNearState()) {
        // Sensor reports that a "handset is being held up to a person's ear",
        // or "something is covering the light sensor".
        setAudioDevice(PeerAudioManager.AudioDevice.EARPIECE);
      } else {
        // Sensor reports that a "handset is removed from a person's ear", or
        // "the light sensor is no longer covered".
        setAudioDevice(PeerAudioManager.AudioDevice.SPEAKER_PHONE);
      }
    }
  }

  /** Construction */
  static public PeerAudioManager create(Context context, Runnable deviceStateChangeListener) {
    return new PeerAudioManager(context, deviceStateChangeListener);
  }

  private PeerAudioManager(Context context, Runnable deviceStateChangeListener) {
  ...
  public void init() {
    ...
    audioManager.setMode(SysAdaptation.GetAudioManageMode());
  ...
  public void setAudioDevice(AudioDevice device) {
    ...
    Utilitys.assertIsTrue(audioDevices.contains(device));
  ...
  private void registerForWiredHeadsetIntentBroadcast() {
    ...
    wiredHeadsetReceiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        ...
        Log.d(TAG, "BroadcastReceiver.onReceive" + Utilitys.getThreadInfo() + ": "
  ...
  private void onAudioManagerChangedState() {
    ...
    if (audioDevices.size() == 2) {
      Utilitys.assertIsTrue(audioDevices.contains(AudioDevice.EARPIECE)
