...
public class TCPLiveCommand implements LiveCommand {
    ...
    private TCPLiveCommand.CmdEvents events;
    private TCPChannel tcpChannel;

    private enum ConnectionState { NEW, CONNECTED, CLOSED, ERROR }

    // All alterations of the room state should be done from inside the looper thread.
    private TCPLiveCommand.ConnectionState roomState;

    public TCPLiveCommand( String uid ) {
        this.events = null;
        hostUserID = uid;
        executor = Executors.newSingleThreadExecutor();
        roomState = TCPLiveCommand.ConnectionState.NEW;
    }
    ...
    @Override
    public void RegisterEvent( TCPLiveCommand.CmdEvents events )
    {
        this.events = events;
    }

    @Override
    public void Present( final String UserName,final SessionDescription sdpOffer ) {
     ...
     *
     */
    public void PresentWithRTMP(final String UserName,final SessionDescription sdpOffer,final String rtmpURL )
    {
    ...
    public void sendLocalIceCandidate(final String uid,final IceCandidate candidate) {
        executor.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (roomState != TCPLiveCommand.ConnectionState.CONNECTED) {
    ...
    */
    ...
    public void onSocketOpen(boolean isServer) {
        if (isServer) {
            roomState = TCPLiveCommand.ConnectionState.CONNECTED;

            LiveCommand.CmdParameters parameters = new LiveCommand.CmdParameters(
    ...
    private void reportError(final String errorMessage) {
        ...
        executor.execute(new Runnable() {
            @Override
            public void run() {
                if (roomState != TCPLiveCommand.ConnectionState.ERROR) {
                    roomState = TCPLiveCommand.ConnectionState.ERROR;
