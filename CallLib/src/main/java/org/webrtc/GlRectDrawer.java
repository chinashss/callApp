...
public class GlRectDrawer implements RendererCommon.GlDrawer {
  ...
  /*
  *   To extend the shader for nv21/line/rectangle drawer by liuhongyu
   */
  private static final String MATRIX_UNIFORM = "uMatrix";
  private static final String POSITION_ATTRIBUTE = "aPosition";

  private static final String YUVN21_FRAGMENT_SHADER_STRING =
          "precision mediump float;\n"
                  + "varying vec2 interp_tc;\n"
                  + "\n"
                  + "uniform sampler2D y_tex;\n"
                  ...
                  // CSC according to http://www.fourcc.org/fccyvrgb.php
                  + "  float y = texture2D(y_tex, interp_tc).r;\n"
                  + "  float u = texture2D(uv_tex, interp_tc).a - 0.5;\n"
                  + "  float v = texture2D(uv_tex, interp_tc).r - 0.5;\n"
                  + "  gl_FragColor = vec4(y + 1.13983 * v, "
   ...
   */
  @Override
  public void drawYuvNV21(int[] yuvTextures, float[] texMatrix, int frameWidth, int frameHeight,
                      int viewportX, int viewportY, int viewportWidth, int viewportHeight) {
    Shader shader = prepareShader(YUVN21_FRAGMENT_SHADER_STRING, texMatrix);
    // Bind the textures.
    for (int i = 0; i < 2; ++i) {
      GLES20.glActiveTexture(GLES20.GL_TEXTURE0 + i);
      GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, yuvTextures[i]);
      //GLES20.glUniform1i(GLES20.gvImageTextureY, i);
    }
    drawRectangle(viewportX, viewportY, viewportWidth, viewportHeight);
    // Unbind the textures as a precaution..
    for (int i = 0; i < 2; ++i) {
    ...
    //shader.glShader.DisableVertextAtrribArray("in_pos");
    //shader.glShader.DisableVertextAtrribArray("in_tc");
  }
  ...
  private Shader prepareShader(String fragmentShader, float[] texMatrix) {
    final Shader shader;
    ...
    } else if (fragmentShader == YUVN21_FRAGMENT_SHADER_STRING) {
      GLES20.glUniform1i(shader.glShader.getUniformLocation("y_tex"), 0);
      GLES20.glUniform1i(shader.glShader.getUniformLocation("uv_tex"), 1);
