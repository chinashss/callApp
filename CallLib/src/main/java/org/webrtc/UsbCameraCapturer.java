...
class UsbCameraCapturer implements CameraVideoCapturer {
    ...
    private final UsbCameraSession.Events cameraSessionEventsHandler = new UsbCameraSession.Events() {
        ...
        */
        ...
        public void onByteBufferFrameCaptured(UsbCameraSession session, byte[] data, int width, int height, int rotation, long timestamp) {
            UsbCameraCapturer.this.checkIsOnCameraThread();
            synchronized (UsbCameraCapturer.this.stateLock) {
                if (session != UsbCameraCapturer.this.currentSession) {
                    Logging.w("UsbCameraCapturer", "onTextureFrameCaptured from another session.");
                    ...
                    UsbCameraCapturer.this.cameraStatistics.addFrame();
                    UsbCameraCapturer.this.capturerObserver.onByteBufferFrameCaptured(data, width, height, rotation, timestamp);
                }
            }
        }

        public void onTextureFrameCaptured(UsbCameraSession session, int width, int height, int oesTextureId, float[] transformMatrix, int rotation, long timestamp) {
            UsbCameraCapturer.this.checkIsOnCameraThread();
            synchronized (UsbCameraCapturer.this.stateLock) {
                if (session != UsbCameraCapturer.this.currentSession) {
                    Logging.w("UsbCameraCapturer", "onTextureFrameCaptured from another session.");
                    ...
                    UsbCameraCapturer.this.cameraStatistics.addFrame();
                    UsbCameraCapturer.this.capturerObserver.onTextureFrameCaptured(width, height, oesTextureId, transformMatrix, rotation, timestamp);
                }
            }
        }
    };
    ...
    public UsbCameraCapturer(String cameraName, @Nullable CameraVideoCapturer.CameraEventsHandler eventsHandler, CameraEnumerator cameraEnumerator) {
